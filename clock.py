...
class Clock:
    ...
    Clock.sleep_win_kernel_adj()      # alternate method
    Clock.sleep_win_kernel_subt()     # alternate method
    Clock.sleep_win_kernel_periodic() # very simple use of an auto resetting timer
...
#        self.mytick=self.tick
    ...
    def sleep_win_kernel_adj(self):
        ...
        r = self.prevtick + self.frame_length - time.perf_counter()
        delay = ctypes.c_longlong(-int((r-self.dly_adj) * 10000000)) # delay must be negative in 100 nanosecond intervals
        n=200.0 # averaging filter for dly_adj
        m=self.prevtick
        ...
        self.prevtick=time.perf_counter()
        self.dly_adj = (self.dly_adj*(n-1) + min((self.prevtick-m)-self.frame_length,0.1*self.frame_length))/n
...
#            print('hi;')
        ...
        # Call the function handle twice to get the internal state correct
        fh()
        fh()
...
#                print(n,sx,sxx)
                ...
                # sys.stdout.write('\r\r{0:8.4f} Hz; {1:8.6f}; {2:8.6f}; {3:8.6f}; {4:8.1f}%     {5:d} {6:f}'.format(1/mean_dt,dt,mean_dt,math.sqrt(sigma/(n+1)),100*duty_f,self.tick,self.subtick/self.frame_len_ns))
                sys.stdout.write(f"\r\r{1/mean_dt:8.4f} Hz; {dt:8.6f}; {mean_dt:8.6f}; {math.sqrt(sigma/(n+1)):8.6f}; {100*duty_f:8.1f}%  {proc_vec[-1]:8.1f}   {self.tick} {self.subtick} ")
                sys.stdout.flush()
#                self.sleep()
...
#                sys.stdout.flush()
        ...
        dtvec = dtvec[1:] # remove first one, its bad
        print(f"\n    mean: {np.mean(dtvec):8.3g}, std dev: {np.std(dtvec):8.5g}, max: {max(dtvec):8.5g}, min:{min(dtvec):8.5g}, processor_load:{np.mean(proc_vec):5.2f}")
        print('\n')
...
if __name__ == "__main__":
    ...
    z=c.test('sleep_ns')
    z=c.test('sleep_b')
    z=c.test('sleep_win_kernel_subt')
    z=c.test('sleep_win_kernel_adj')
    z=c.test('sleep_win_kernel_periodic')
